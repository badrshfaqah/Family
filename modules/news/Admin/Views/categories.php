...
?>
<div class="card-box">
  <form method="post" action="<?= Url::admin('news-categories/store') ?>" style="display:flex;gap:10px">
    <?= Csrf::field() ?>
  ...
  <table>
    ...
    <?php foreach ($categories as $c): ?>
      <tr>
        ...
        <td><form method="post" action="<?= Url::admin('news-categories/' . $c['id'] . '/delete') ?>" data-confirm="حذف هذا التصنيف؟"><?= Csrf::field() ?><button class="btn btn-danger btn-sm" type="submit">حذف</button></form></td>
      </tr>
    <?php endforeach; ?>
