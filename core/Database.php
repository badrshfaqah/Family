...
final class Database
{
    ...
    public static function connect(string $host, string $port, string $name, string $user, string $pass, string $charset = 'utf8mb4', string $prefix = ''): PDO
    {
        ...
        // مزامنة المنطقة الزمنية لجلسة MySQL مع منطقة PHP الزمنية المُعدّة (APP_TIMEZONE)،
        // وإلا فإن NOW() في MySQL قد تختلف ساعات عن date() في PHP فتُخفي محتوى "منشورًا للتو".
        $offset = date('P');
        ...
        self::$pdo = new PDO($dsn, $user, $pass, [
            ...
            PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES {$charset} COLLATE {$charset}_unicode_ci, time_zone = '{$offset}'",
        ]);
